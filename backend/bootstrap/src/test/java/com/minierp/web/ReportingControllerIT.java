...
@ActiveProfiles("test")
@DisplayName("ReportingController — HTTP layer")
class ReportingControllerIT {
    ...
    void seed() {
        ...
        token = jwtService.issueAccessToken(new CurrentUser(
                ...
                Set.of(), Set.of("reporting:read")));
    }

    @Test
    @DisplayName("GET /reports/direction returns 200")
    void directionReport_returns200() throws Exception {
        mockMvc.perform(get("/api/v1/reports/direction").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk());
    }

    @Test
    @DisplayName("GET /reports/stock returns 200")
    void stockReport_returns200() throws Exception {
        mockMvc.perform(get("/api/v1/reports/stock").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk());
    }
}
