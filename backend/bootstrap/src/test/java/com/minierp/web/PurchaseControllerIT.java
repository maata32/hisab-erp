...
@ActiveProfiles("test")
@DisplayName("Purchase controllers (PurchaseOrder/PurchaseInvoice) — HTTP layer")
class PurchaseControllerIT {
    ...
    void seed() {
        ...
        token = jwtService.issueAccessToken(new CurrentUser(
                ...
                Set.of(), Set.of("purchase:read", "purchase:create", "purchase:update", "purchase:receive")));
    }

    @Test
    @DisplayName("GET /purchase-orders returns 200")
    void listPurchaseOrders_returns200() throws Exception {
        mockMvc.perform(get("/api/v1/purchase-orders").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content").isArray());
    }

    @Test
    @DisplayName("GET /purchase-invoices returns 200")
    void listPurchaseInvoices_returns200() throws Exception {
        mockMvc.perform(get("/api/v1/purchase-invoices").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content").isArray());
    }
