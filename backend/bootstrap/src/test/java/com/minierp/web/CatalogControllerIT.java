...
@ActiveProfiles("test")
@DisplayName("Catalog controllers (Brand + ProductCategory) — HTTP layer")
class CatalogControllerIT {
    ...
    void seed() {
        ...
        token = jwtService.issueAccessToken(new CurrentUser(
                ...
                Set.of(), Set.of("product:read", "product:create", "product:update")));
    }

    @Test
    @DisplayName("GET /brands returns 200")
    void listBrands_returns200() throws Exception {
        mockMvc.perform(get("/api/v1/brands").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk());
    }

    @Test
    @DisplayName("GET /product-categories returns 200")
    void listCategories_returns200() throws Exception {
        mockMvc.perform(get("/api/v1/product-categories").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk());
    }
}
