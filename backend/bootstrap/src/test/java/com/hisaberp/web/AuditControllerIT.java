...
@ActiveProfiles("test")
@DisplayName("AuditController — HTTP layer")
class AuditControllerIT {
    ...
    void seed() {
        ...
        token = jwtService.issueAccessToken(new CurrentUser(
                ...
                Set.of("MANAGER"), Set.of("audit:read")));

        tokenWithoutPerm = jwtService.issueAccessToken(new CurrentUser(
                UUID.randomUUID(), tenantId, "no-audit@test.local", "fr",
                Set.of(), Set.of("product:read")));
    }

    @Test
    @DisplayName("GET /audit with audit:read returns 200 (verifies fix from role->authority migration)")
    void listAudit_withPermission_returns200() throws Exception {
    ...
    @Test
    @DisplayName("GET /audit without audit:read returns 403")
    void listAudit_withoutPermission_returns403() throws Exception {
        mockMvc.perform(get("/api/v1/audit").header("Authorization", "Bearer " + tokenWithoutPerm))
